...
int main_0() {
  ...
  __CPROVER_assume(!(((q * _x) - (s * r)) != 1));
  __CPROVER_assume(!(b != ((tmp_var_2_int * q) + (local_4c * s))));
  __CPROVER_assume(!(p != ((local_4c * _x) + (tmp_var_2_int * r))));
  __CPROVER_assume(!(b == p));
  __CPROVER_assume(!(p < b));
  p = p - b;
  r = r - q;
  _x = _x - s;
  iVar2 = counter + 1;
  ...
  __assert_fail("0 && \"a - b == 0\"", "/home/zyl/biVerify/CPA_DATA2/C/F/llmfix/realOutput/iterationTry/egcd-ll_unwindbound1_process_dc_iter1.c", 62, "__PRETTY_FUNCTION__");
}
