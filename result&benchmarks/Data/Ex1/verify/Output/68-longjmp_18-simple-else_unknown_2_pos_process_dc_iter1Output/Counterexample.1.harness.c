...
void _longjmp(struct __jmp_buf_tag __env[1], int __val) {
  fprintf(stderr, "Called method _longjmp that has attribute noreturn.\n");
  exit(1);
}
...
void longjmp(struct __jmp_buf_tag __env[1], int __val) {
  fprintf(stderr, "Called method longjmp that has attribute noreturn.\n");
  exit(1);
}
long int lrand48() {
  long int retval;
...
void siglongjmp(sigjmp_buf __env, int __val) {
  fprintf(stderr, "Called method siglongjmp that has attribute noreturn.\n");
  exit(1);
}
