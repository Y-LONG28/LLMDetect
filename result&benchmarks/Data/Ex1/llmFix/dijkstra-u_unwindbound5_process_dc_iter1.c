...
int main(void) {
    ...
    assume((uint)(local_24 < 0x3fffffff));
    r = 0;
    tmp_var_1_uint = local_24;
    n = 0;

    for (h = 1; (iVar2 = counter + 1, bVar1 = counter < 5, counter = iVar2, bVar1 && (h <= local_24)); h = h << 2) {
    }
    ...
    while (iVar2 = counter + 1, bVar1 = counter < 5, counter = iVar2, bVar1) {
        ...
        if (tmp_var_1_uint * h * tmp_var_1_uint * 4 +
            ...
            ((h * local_24 * local_24 * 4 + local_24 * n * n + r * n * local_24 * -4) - h * local_24 * h) != 0) {
            assert(0 && "h * h * n - 4 * h * n * p + 4 * (n * n) * q - n * q * q - h * h * r + 4 * h * p * r - 8 * n * q * r + q * q * r + 4 * q * r * r == 0");
        }
        if ((tmp_var_1_uint * n * h * 4 - h * r * h) +
            h * local_24 * r * 4 + r * n * n + h * n * local_24 * -4 != tmp_var_1_uint * r * h * 4) {
            assert(0 && "h * h * p - 4 * h * n * q + 4 * n * p * q - p * q * q + 4 * h * q * r - 4 * p * q * r == 0");
        }
        if (h * tmp_var_1_uint + (r * r - local_24 * h) != 0) {
            assert(0 && "p * p - n * q + q * r == 0");
        }
        if (h == 1) break;
        h = h >> 2;
        n = h + r;
    ...
    if ((local_24 * r * 0x10 + n * tmp_var_1_uint * 0xc + n * n * n + n * local_24 * -0xc +
        ...
        assert(0 && "h*h*h - 12*h*n + 16*n*p + 12*h*r - 16*p*r - h - 4*p == 0");
    }
    ...
    if (r != (local_24 * r + n * tmp_var_1_uint) * 4 + r * n * n + n * local_24 * -4 +
        r * tmp_var_1_uint * -4) {
        assert(0 && "h*h*p - 4*h*n + 4*n*p + 4*h*r - 4*p*r - p == 0");
    }
